...
print "******************************************************************"
print "This script is going to take you through the steps to check in a"
print "\tbook to the DE library."
print "******************************************************************"
...
#there is a single row element in table with this NFC id (proceed)
    ...
    confirm = raw_input("Is this the book you want to check in? [y/n]\n>> ")
    ...
    #check that the status of the field is not already 0 (checked in)
    if rows[0] == 0:
        print "ERR: this item is currently already checked in!"
        print "Book needs to be checked out in order to be able to check back in..."
        cursor.close()
        db.close()
        sys.exit()
    #status field is 1 -> we can change it to 0 and set the date field
    else:
        try:
            ret = cursor.execute("UPDATE booklist SET status=%s, date=%s WHERE nfc_id=%s;",
                                ('0', time.strftime('%Y-%m-%d'), str(RFID)))
        except:
